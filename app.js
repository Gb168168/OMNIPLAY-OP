...
function files(p,folderId=null,childId=null){const w=$('#workspace'),folder=(p.folders||[]).find(item=>item.id===folderId),child=(folder?.folders||[]).find(item=>item.id===childId),target=child||folder||p;
w.className='workspace';p.files||=[];p.folders||=[];if(folder)folder.folders=folder.folders||[];target.files=target.files||[];
const visibleFolders=child?[]:folder?(folder.folders||[]):p.folders,folderCards=visibleFolders.map(item=>`<button type="button" class="file-card workspace-folder-card" data-open-folder="${esc(item.id)}"><div class="file-placeholder">${item.type==='photos'?'🖼️':'📁'}</div><div class="file-name">${esc(item.name)}</div><small>${item.type==='photos'?'圖片資料夾':'檔案資料夾'} · ${(item.files||[]).length} 項</small></button>`).join('');
const title=child?`${folder.name} / ${child.name}`:folder?folder.name:'GAME ID 主資料夾';
w.innerHTML=`<div class="notice">☁️ ${folder?`<button type="button" class="secondary" id="folderBack">← 返回</button>　<strong>${esc(title)}</strong>`:'點選下方資料夾管理個別檔案／圖片。'}</div><div class="file-panel">${folder||!p.folders.length?'<div id="uploadZone" class="upload-zone"><strong>＋ 加入檔案／圖片</strong><small>也可以點此後按 Ctrl + V 貼上圖片</small></div>':''}<div id="fileGrid" class="file-grid">${folderCards}</div></div>`;
if(folder)$('#folderBack').onclick=()=>child?files(p,folderId):files(p);
w.querySelectorAll('[data-open-folder]').forEach(button=>button.onclick=()=>folder?files(p,folderId,button.dataset.openFolder):files(p,button.dataset.openFolder));
const addFiles=list=>{[...list].forEach(file=>target.files.push({id:uid('file'),name:file.name,type:file.type,size:file.size}));save();files(p,folderId,childId)},upload=$('#uploadZone');if(upload){upload.tabIndex=0;upload.onclick=()=>$('#hiddenUpload').click();upload.onpaste=e=>{const pasted=[...e.clipboardData.items].filter(item=>item.kind==='file').map(item=>item.getAsFile()).filter(Boolean);if(pasted.length){e.preventDefault();addFiles(pasted)}};$('#hiddenUpload').onchange=e=>{addFiles(e.target.files);e.target.value=''}}
const g=$('#fileGrid');target.files.forEach(file=>g.insertAdjacentHTML('beforeend',`<div class="file-card">${file.dataUrl?`<img src="${esc(file.dataUrl)}" alt="" style="width:100%;height:100px;object-fit:cover;border-radius:8px">`:`<div class="file-placeholder">${file.type?.startsWith('image/')?'🖼️':'📄'}</div>`}<div class="file-name">${esc(file.name)}</div></div>`))}
