...
function mergeSelectedCells(action){if(!currentUniver)return;try{const workbook=currentUniver.api.getActiveWorkbook(),worksheet=workbook?.getActiveSheet?.(),range=worksheet?.getActiveRange?.()||workbook?.getActiveRange?.();if(!range)throw new Error('請先選取儲存格');if(action==='merge')range.merge();else range.breakApart();$('#sheetMergeStatus').textContent=action==='merge'?'已合併':'已取消合併';scheduleSheetSave()}catch(e){console.warn('merge cells',e);$('#sheetMergeStatus').textContent=e?.message||'操作失敗'}}
function sheet(p){const w=$('#workspace');
w.className='workspace sheet-workspace';
w.innerHTML='<div class="sheet-note"><span>📊 試算表・繁體中文・Firestore 雲端同步</span><div class="sheet-cell-tools"><button type="button" id="sheetMergeBtn">合併儲存格</button><button type="button" id="sheetUnmergeBtn">取消合併</button><small id="sheetMergeStatus"></small></div></div><div id="univerSheet" class="univer-sheet"></div>';
...
currentUniver={univer:x.univer,api:x.univerAPI,page:p};currentUniver.autoSaveDisposable=x.univerAPI.addEvent?.(x.univerAPI.Event.CommandExecuted,()=>scheduleSheetSave());$('#sheetMergeBtn').onclick=()=>mergeSelectedCells('merge');$('#sheetUnmergeBtn').onclick=()=>mergeSelectedCells('unmerge');applyVisibleCellFixes(x.univerAPI,String(p.name||'').trim())}catch(e){w.innerHTML=`<div class="notice">試算表載入失敗：${esc(e.message)}</div>`}}
