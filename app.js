...
const GROUP_PERMISSION_DEFAULT_VERSION=1;function isGroupPermissionPage(category,page){const name=String(category?.name||'').trim();return name!=='內部參考，請勿外流'&&name!=='所有平台列表'&&!isLegacyGameAssetPage(category,page)}function workspacePageIds(){return(state.categories||[]).flatMap(category=>(category.pages||[]).filter(page=>isGroupPermissionPage(category,page)).map(page=>page.id))}function newCustomerGroup(name){const pageIds=workspacePageIds();return{id:uid('grp'),name,allowedPages:[...pageIds],pageOrder:[...pageIds],permissionMode:'all',permissionDefaultsVersion:GROUP_PERMISSION_DEFAULT_VERSION}}function initializeGroupPermissions(){const pageIds=workspacePageIds(),eligible=new Set(pageIds);let changed=false;for(const group of state.customerGroups||[]){const allowed=group.allowedPages||[];if(group.permissionDefaultsVersion!==GROUP_PERMISSION_DEFAULT_VERSION){group.allowedPages=[...pageIds];group.pageOrder=[...pageIds];group.permissionMode='all';group.permissionDefaultsVersion=GROUP_PERMISSION_DEFAULT_VERSION;changed=true;continue}const nextAllowed=group.permissionMode==='all'?[...pageIds]:allowed.filter(id=>eligible.has(id)),nextOrder=group.permissionMode==='all'?[...pageIds]:(group.pageOrder||[]).filter(id=>eligible.has(id));if(JSON.stringify(allowed)!==JSON.stringify(nextAllowed)||JSON.stringify(group.pageOrder||[])!==JSON.stringify(nextOrder))changed=true;group.allowedPages=nextAllowed;group.pageOrder=nextOrder}return changed}
