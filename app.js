...
const GROUP_PERMISSION_DEFAULT_VERSION=2;function isVisibleGroupPermissionPage(category,page){return!isLegacyGameAssetPage(category,page)}function isDefaultGroupPermissionPage(category,page){const name=String(category?.name||'').trim();return isVisibleGroupPermissionPage(category,page)&&name!=='內部參考，請勿外流'&&name!=='所有平台列表'}function workspacePageIds(){return(state.categories||[]).flatMap(category=>(category.pages||[]).filter(page=>isVisibleGroupPermissionPage(category,page)).map(page=>page.id))}function defaultGroupPageIds(){return(state.categories||[]).flatMap(category=>(category.pages||[]).filter(page=>isDefaultGroupPermissionPage(category,page)).map(page=>page.id))}function newCustomerGroup(name){const pageIds=defaultGroupPageIds();return{id:uid('grp'),name,allowedPages:[...pageIds],pageOrder:[...workspacePageIds()],permissionMode:'all',permissionDefaultsVersion:GROUP_PERMISSION_DEFAULT_VERSION}}function initializeGroupPermissions(){const allIds=workspacePageIds(),defaultIds=defaultGroupPageIds(),visible=new Set(allIds);let changed=false;for(const group of state.customerGroups||[]){const allowed=group.allowedPages||[],platformAccess=isInternalGroup(group)&&allowed.includes(PLATFORM_LIST_PERMISSION);if(group.permissionDefaultsVersion!==GROUP_PERMISSION_DEFAULT_VERSION){group.allowedPages=[...defaultIds];group.pageOrder=[...allIds,...(isInternalGroup(group)?[PLATFORM_LIST_PERMISSION]:[])];group.permissionMode='all';group.permissionDefaultsVersion=GROUP_PERMISSION_DEFAULT_VERSION;changed=true;continue}const nextAllowed=group.permissionMode==='all'?[...defaultIds,...(platformAccess?[PLATFORM_LIST_PERMISSION]:[])]:allowed.filter(id=>visible.has(id)||(id===PLATFORM_LIST_PERMISSION&&isInternalGroup(group))),nextOrder=[...allIds,...(isInternalGroup(group)?[PLATFORM_LIST_PERMISSION]:[])];if(JSON.stringify(allowed)!==JSON.stringify(nextAllowed)||JSON.stringify(group.pageOrder||[])!==JSON.stringify(nextOrder))changed=true;group.allowedPages=nextAllowed;group.pageOrder=nextOrder}return changed}
...
function files(p,path=[]){const w=$('#workspace'),canEdit=isAdminWorkspaceView();p.files||=[];p.folders||=[];let target=p,validPath=[],names=[];for(const id of path){target.folders=target.folders||[];const next=target.folders.find(folder=>folder.id===id);if(!next)break;target=next;validPath.push(id);names.push(next.name)}target.files=target.files||[];target.folders=target.folders||[];w.className='workspace';const crumbs=[{name:p.name,path:[]}].concat(names.map((name,index)=>({name,path:validPath.slice(0,index+1)}))),folderCards=target.folders.map(folder=>`<button type="button" class="file-card workspace-folder-card" data-open-folder="${esc(folder.id)}"><div class="file-placeholder">${folder.type==='photos'?'🖼️':'📁'}</div><div class="file-name">${esc(folder.name)}</div><small>${folder.type==='photos'?'圖片資料夾':'檔案資料夾'} · ${(folder.files||[]).length} 項</small></button>`).join('');w.innerHTML=`<div class="notice">☁️ ${crumbs.map((crumb,index)=>`${index?' › ':''}<button type="button" class="secondary" data-folder-crumb="${index}">${esc(crumb.name)}</button>`).join('')}</div><div class="file-panel">${validPath.length&&canEdit?'<div id="uploadZone" class="upload-zone" tabindex="0"><strong>＋ 加入檔案／圖片</strong><small>也可以點此後按 Ctrl + V 貼上圖片</small></div>':''}<div id="fileGrid" class="file-grid">${folderCards}</div></div>`;w.querySelectorAll('[data-folder-crumb]').forEach(button=>button.onclick=()=>files(p,crumbs[Number(button.dataset.folderCrumb)].path));w.querySelectorAll('[data-open-folder]').forEach(button=>button.onclick=()=>files(p,[...validPath,button.dataset.openFolder]));const addFiles=list=>{if(!canEdit)return alert('只有管理員可以新增或上傳檔案');[...list].forEach(file=>target.files.push({id:uid('file'),name:file.name,type:file.type,size:file.size}));save();files(p,validPath)},upload=$('#uploadZone');if(upload){upload.onclick=()=>$('#hiddenUpload').click();upload.onpaste=e=>{const pasted=[...e.clipboardData.items].filter(item=>item.kind==='file').map(item=>item.getAsFile()).filter(Boolean);if(pasted.length){e.preventDefault();addFiles(pasted)}};$('#hiddenUpload').onchange=e=>{addFiles(e.target.files);e.target.value=''}}const g=$('#fileGrid');target.files.forEach(file=>g.insertAdjacentHTML('beforeend',`<div class="file-card">${file.dataUrl?`<img src="${esc(file.dataUrl)}" alt="" style="width:100%;height:100px;object-fit:cover;border-radius:8px">`:`<div class="file-placeholder">${file.type?.startsWith('image/')?'🖼️':'📄'}</div>`}<div class="file-name">${esc(file.name)}</div></div>`))}
...
function renderCustomers(){if(!canAccessPlatformList()){alert('此帳號沒有「所有平台列表」權限');renderPage();return}dispose();rememberView('customers');$('#customerBtn').classList.add('active');document.querySelectorAll('.category-head.active').forEach(element=>element.classList.remove('active'));
...
customers.forEach(u=>{const selectedGroup=groups.find(g=>g.id===u.groupId);const tr=document.createElement('tr');tr.innerHTML=`<td><strong>${esc(u.name||'未命名')}</strong></td><td>${esc(u.domain||u.username||'—')}</td><td>${esc(u.customerType||'未設定')}</td><td>${esc(u.progress||'未設定')}</td><td>${esc(formatLaunchDate(u.launchDate))}</td><td><div class="platform-notes" title="${esc(u.notes||'')}">${esc(u.notes||'—')}</div></td><td>${esc(selectedGroup?.name||'未設定')}</td><td><div class="platform-actions"><button type="button" class="platform-edit">編輯</button><button type="button" class="permission-btn">設定權限</button><button type="button" class="customer-delete">刪除</button></div></td>`;
...
function allInternalMembers(){return state.customerGroups.flatMap(group=>(group.members||[]).map(member=>({...member,groupId:group.id})))}
function ensureGroupMembersDialog(){let dialog=$('#groupMembersDialog');if(dialog)return dialog;dialog=document.createElement('dialog');dialog.id='groupMembersDialog';dialog.className='modal-xl';document.body.append(dialog);return dialog}
function openGroupMembers(g,editingId=null){g.members||=[];const editing=g.members.find(member=>member.id===editingId),dialog=ensureGroupMembersDialog();dialog.innerHTML=`<div class="dialog-form group-members-dialog"><div class="modal-head"><div><div class="eyebrow">INTERNAL MEMBERS</div><h3>${esc(g.name)}－人員管理</h3><p>管理此群組的人員與登入帳密。</p></div><button type="button" class="icon-close" data-member-close>×</button></div><form id="groupMemberForm" class="group-member-form"><label>人員名稱<input id="groupMemberName" required maxlength="60" value="${esc(editing?.name||'')}" placeholder="輸入姓名"/></label><label>登入帳號<input id="groupMemberUsername" required maxlength="80" value="${esc(editing?.username||'')}" autocomplete="off" placeholder="輸入登入帳號"/></label><label>登入密碼<input id="groupMemberPassword" required type="password" maxlength="120" value="${esc(editing?.password||'')}" autocomplete="new-password" placeholder="輸入登入密碼"/></label><button class="primary" type="submit">${editing?'儲存變更':'＋ 新增人員'}</button>${editing?'<button class="secondary" type="button" id="cancelMemberEdit">取消編輯</button>':''}</form><div class="group-member-list">${g.members.length?g.members.map(member=>`<div class="group-member-row"><div class="customer-avatar">${esc((member.name||'?').slice(0,1).toUpperCase())}</div><div class="group-member-main"><strong>${esc(member.name)}</strong><span>帳號：${esc(member.username)}</span></div><button type="button" class="group-member-edit" data-member-edit="${member.id}">編輯</button><button type="button" class="group-member-delete" data-member-delete="${member.id}">刪除</button></div>`).join(''):'<div class="empty-mini"><div>👤</div><strong>尚未新增內部人員</strong><span>請使用上方欄位建立第一個帳號</span></div>'}</div><div class="dialog-actions"><button type="button" class="secondary" data-member-close>關閉</button></div></div>`;dialog.querySelectorAll('[data-member-close]').forEach(button=>button.onclick=()=>dialog.close());dialog.querySelector('#cancelMemberEdit')?.addEventListener('click',()=>openGroupMembers(g));dialog.querySelectorAll('[data-member-edit]').forEach(button=>button.onclick=()=>openGroupMembers(g,button.dataset.memberEdit));dialog.querySelectorAll('[data-member-delete]').forEach(button=>button.onclick=()=>{const member=g.members.find(item=>item.id===button.dataset.memberDelete);if(!member||!confirm(`確定要刪除人員「${member.name}」嗎？`))return;g.members=g.members.filter(item=>item.id!==member.id);save();openGroupMembers(g)});dialog.querySelector('#groupMemberForm').onsubmit=event=>{event.preventDefault();const name=$('#groupMemberName').value.trim(),username=$('#groupMemberUsername').value.trim(),password=$('#groupMemberPassword').value;if(!name||!username||!password)return;const duplicate=allInternalMembers().find(member=>member.id!==editingId&&String(member.username||'').trim().toLowerCase()===username.toLowerCase());if(duplicate){alert('此登入帳號已被其他人員使用');return}if(editing)Object.assign(editing,{name,username,password,updatedAt:new Date().toISOString()});else g.members.push({id:uid('member'),name,username,password,createdAt:new Date().toISOString()});save();openGroupMembers(g)};if(!dialog.open)dialog.showModal();setTimeout(()=>$('#groupMemberName')?.focus(),50)}
function openGroupManager(){const list=$('#groupManagerList');list.innerHTML='';if(!state.customerGroups.length)list.innerHTML='<div class="empty-mini"><div>👥</div><strong>尚無群組分類</strong><span>按下方按鈕新增第一個群組</span></div>';state.customerGroups.forEach(g=>{const count=state.customers.filter(u=>u.groupId===g.id).length,row=document.createElement('div');row.className='group-manager-row';row.innerHTML=`<div class="group-icon">👥</div><div class="group-manager-main"><strong>${esc(g.name)}</strong><span>${count} 個平台・可看 ${(g.allowedPages||[]).length} 個頁面</span></div><div class="group-manager-actions"><button type="button" class="group-members">人員管理</button><button type="button" class="group-permissions">權限設定</button><button type="button" class="group-edit">編輯</button><button type="button" class="group-delete" ${count?'disabled':''}>刪除</button></div>`;row.querySelector('.group-members').onclick=()=>openGroupMembers(g);row.querySelector('.group-permissions').onclick=()=>{$('#groupManagerDialog').close();editGroupPermissions(g)};row.querySelector('.group-edit').onclick=()=>{const name=prompt('編輯群組名稱：',g.name)?.trim();if(!name||name===g.name)return;if(state.customerGroups.some(group=>group.id!==g.id&&String(group.name||'').trim().toLowerCase()===name.toLowerCase())){alert('已有相同名稱的群組');return}g.name=name;save();openGroupManager()};const del=row.querySelector('.group-delete');if(count)del.title='請先將此群組的平台移到其他群組';del.onclick=()=>{if(count){alert(`「${g.name}」仍有 ${count} 個平台使用中，請先將平台移到其他群組。`);return}if(!confirm(`確定要刪除群組「${g.name}」嗎？\n刪除後無法復原。`))return;state.customerGroups=state.customerGroups.filter(group=>group.id!==g.id);save();openGroupManager()};list.append(row)});const dialog=$('#groupManagerDialog');if(!dialog.open)dialog.showModal()}
...
if(t!=='sheet')p.files=[];
c.pages.push(p);
state.customerGroups.forEach(group=>{if(group.permissionMode==='all'&&isDefaultGroupPermissionPage(c,p)){group.allowedPages=group.allowedPages||[];group.pageOrder=group.pageOrder||[];if(!group.allowedPages.includes(p.id))group.allowedPages.push(p.id);if(!group.pageOrder.includes(p.id))group.pageOrder.push(p.id)}});
state.activePageId=p.id;
save();
$('#pageDialog').close();
renderNav();
