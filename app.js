...
async function hydrateSheetSnapshots(){for(const category of state.categories||[]){for(const page of category.pages||[]){if(page.type!=='sheet'||!page.id)continue;try{const stored=await getDoc(sheetRef(page.id));if(stored.exists()&&stored.data()?.snapshot)page.snapshot=stored.data().snapshot;else if(page.snapshot){const snapshot=cleanSnapshot(page.snapshot);if(snapshot)await setDoc(sheetRef(page.id),{snapshot,updatedAt:new Date().toISOString()})}}catch(e){console.warn('load sheet snapshot',page.id,e)}}}}
async function persistSheetSnapshot(page,snapshot=page?.snapshot){if(!page?.id||!snapshot)return false;const clean=cleanSnapshot(snapshot);if(!clean)return false;page.snapshot=clean;await setDoc(sheetRef(page.id),{snapshot:clean,updatedAt:new Date().toISOString()});return true}
