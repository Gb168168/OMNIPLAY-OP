...
function formatJackpotLines(cell){let current=cell;for(let i=0;i<4&&current&&typeof current==='object'&&'v'in current;i++){if(typeof current.v==='string'){current.v=current.v.replace(/\s+(Major\s*:)/gi,'\n$1');break}current=current.v}return cell}
function formatNumericCell(cell,header){if(!cell||typeof cell!=='object'||typeof cell.v!=='string')return cell;const numericColumn=/^(no of lines|bet php|maximum|minimum|max prize|jackpot range|jackpot rtp|total jackpot rtp|base game rtp|total payout)/.test(header),raw=cell.v.trim();if(!numericColumn||!/^[+-]?[\d,]+(?:\.\d+)?$/.test(raw))return cell;const value=Number(raw.replace(/,/g,''));if(!Number.isFinite(value))return cell;cell.v=value;cell.t=2;cell.s=cell.s&&typeof cell.s==='object'?{...cell.s,n:{pattern:'#,##0.###'}}:{n:{pattern:'#,##0.###'}};return cell}
...
const sourceCells=source.cellData||{},targetCells=target.cellData||(target.cellData={}),sourceHeader=sourceCells[0]||{},targetHeader=targetCells[0]||{};
...
let lastRow=1,changed=true;Object.entries(sourceCells).sort((a,b)=>+a[0]-+b[0]).forEach(([sourceRow,cells])=>{const rowNumber=+sourceRow;if(rowNumber===0||!cells)return;const row=targetCells[rowNumber]||(targetCells[rowNumber]={});let copied=false;matches.forEach(([from,to])=>{const sourceCell=cells[from];if(sourceCell==null)return;const cloned=typeof structuredClone==='function'?structuredClone(sourceCell):JSON.parse(JSON.stringify(sourceCell)),header=normalizedHeader(cellValue(targetHeader[to]));row[to]=formatNumericCell(formatJackpotLines(cloned),header);copied=true});if(copied)lastRow=Math.max(lastRow,rowNumber)});
